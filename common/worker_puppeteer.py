    ...
    while True:
        for message in COLA_PUPPETEER.receive_messages(MaxNumberOfMessages=1, MessageAttributeNames=['Id']):
            if message.message_attributes is not None:
                ...
                nuevo_archivo = util.copiar_contenido(resultado, settings.PUPPETEER_PATH, settings.RUTAS_INTERNAS["Puppeteer"], '.js')

                salida = subprocess.call(['jest', nuevo_archivo], shell=True, cwd=settings.PUPPETEER_PATH)
